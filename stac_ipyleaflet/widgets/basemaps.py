...
class BasemapsWidget:
    def template(self, **kwargs) -> Box():
        ...
        def make_base_layer(url=None, visible=False, name="", layer=""):
            if url:
                layer = TileLayer(
                    url=url, name=name, base=True, visible=False, **kwargs
                )
                return layer
            else:
        ...
        basemap_layers = [
            {"layer": basemaps.Esri.WorldImagery, "name": "Esri World Imagery"},
        ...
        for bm in basemap_layers:
            if bm["name"] == "Open Street Map":
                bm_layer = make_base_layer(
                    layer=bm["layer"], name=bm["name"], visible=True
                )
            else:
